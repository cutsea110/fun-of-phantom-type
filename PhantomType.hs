...
-- imap (f <*> g) (RInt) i = i
-- (imap f <*> imap g) (RInt) i = (imap f (RInt) . imap g (RInt)) i
--                              = imap f (RInt) $ imap g (RInt) i
--                              = imap f (RInt) i
--                              = i
-- imap (f <*> g) (RChar) c = c
-- (imap f <*> imap g) (RChar) c = (imap f (RChar) . imap g (RChar)) c
--                               = imap f (RChar) $ imap g (RChar) c
--                               = imap f (RChar) c
--                               = c
-- imap (f <*> g) (RList ra) [] = []
-- (imap f <*> imap g) (RList ra) [] = (imap f (RList ra) . imap g (RList ra)) []
--                                   = imap f (RList ra) $ imap g (RList ra) []
--                                   = imap f (RList ra) []
--                                   = []
-- imap (f <*> g) (RList ra) (a:as) = (f <*> g) ra a:(f <*> g) (RList ra) as
--                                  = (f ra . g ra) a:(f (RList ra) . g (RList ra)) as
--                                  = (f ra $ g ra a):(f (RList ra) $ g (RList ra) as)
-- (imap f <*> imap g) (RList ra) (a:as) = (imap f (RList ra) . imap g (RList ra)) (a:as)
--                                       = imap f (RList ra) $ imap g (RList ra) (a:as)
--                                       = imap f (RList ra) $ g ra a:g (RList ra) as
--                                       = f ra (g ra a):f (RList ra) (g (RList ra) as)
--                                       = (f ra $ g ra a):(f (RList ra) $ g (RList ra) as)
-- imap (f <*> g) (RPair ra rb) (a, b) = ((f <*> g) ra a, (f <*> g) rb b)
--                                     = ((f ra . g ra) a, (f rb . g rb) b)
--                                     = (f ra $ g ra a, f rb $ g rb b)
-- (imap f <*> imap g) (RPair ra rb) (a, b) = (imap f (RPair ra rb) . imap g (RPair ra rb)) (a, b)
--                                          = imap f (RPair ra rb) $ imap g (RPair ra rb) (a, b)
--                                          = imap f (RPair ra rb) (g ra a, g rb b)
--                                          = (f ra (g ra a), f rb (g rb b))
--                                          = (f ra $ g ra a, f rb $ g rb b)
-- imap (f <*> g) (RPerson) (Person n a) = Person ((f <*> g) rString n) ((f <*> g) RInt a)
--                                       = Person ((f rString . g rString) n) ((f RInt . g RInt) a)
--                                       = Person (f rString $ g rString n) (f RInt $ g RInt a)
-- (imap f <*> imap g) (RPerson) (Person n a) = (imap f (RPerson) . imap g (RPerson)) (Person n a)
--                                            = imap f (RPerson) $ imap g (RPerson) (Person n a)
--                                            = imap f (RPerons) $ Person (g rString n) (g RInt a)
--                                            = Person (f rString (g rString n)) (f RInt (g RInt a))
--                                            = Person (f rString $ g rString n) (f RInt $ g RInt a)
